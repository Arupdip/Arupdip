...
$(document).ready(function(){
    ...
    if($('.datatableadv').length > 0){
        $('.datatableadv').DataTable({
            // scrollY: 100,
            "language": {
                "paginate": {
            ...
            "initComplete": function(settings, json) {
                $('table.datatableadv').wrap('<div class="table-responsive"></div>');
            }
        });
    }

    if($('.pri-off').length > 0){
        $('.pri-off').find('input,textarea,select').attr('tabindex',-1);
    }
